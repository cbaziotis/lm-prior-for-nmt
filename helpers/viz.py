...
def seq2seq_attentions(x, y, a, file, title=""):
    from matplotlib import rc
    # rc('font', **{'family': 'serif', 'serif': ['CMU Serif']})

    with PdfPages(file) as pdf:
        for src, trg, att in zip(x, y, a):
            att = numpy.array(att).T
            height = round(math.log(math.pow(len(src), 2))) + 1
            width = round(math.log(math.pow(len(trg), 2))) + 1
            fig, ax = plt.subplots(1, 1, figsize=(width, height))
            attention_heatmap_subplot(src, trg, att, ax=ax)
            ax.set_title(title)
            fig.tight_layout()
            pdf.savefig(fig)
            plt.close(fig)
