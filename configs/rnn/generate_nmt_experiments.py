def get_name(src, trg, model, ls=False, synthetic=False, subsample=0):
    name = f"final.rnn.{src}{trg}_{model}"
...
def fusion(pair, method, ls, bt, synth, subsample=0):
    ...
    cmd += f"../../checkpoints/prior.lm_news_{trg}_3M_rnn_big_best.pt"
...
def prior(pair, ls, bt, synth, subsample=0, obj="kl", size="30M"):
    ...
    # specify which prior to use
    cmd += f" data.prior_path="
    cmd += f"../../checkpoints/prior.lm_news_{trg}_{size}_rnn_big_best.pt"
...
def lang_pair_experiments(p, bt, syn, subsample):
    ...
    # baseline
    exps.append(baseline(p, ls=False, bt=bt, synth=syn, subsample=subsample))

    # baseline + LS
    exps.append(baseline(p, ls=True, bt=bt, synth=syn, subsample=subsample))

    # LM-fusion
    exps.append(fusion(p, "prenorm", ls=False, bt=bt, synth=syn, subsample=subsample))
    exps.append(fusion(p, "postnorm", ls=False, bt=bt, synth=syn, subsample=subsample))

    # LM-fusion + LS
    exps.append(fusion(p, "prenorm", ls=True, bt=bt, synth=syn, subsample=subsample))
    exps.append(fusion(p, "postnorm", ls=True, bt=bt, synth=syn, subsample=subsample))

    # LM-Prior
    exps.append(prior(p, ls=False, bt=bt, synth=syn, obj="kl", size="3M", subsample=subsample))

    # LM-Prior + LS
    exps.append(prior(p, ls=True, bt=bt, synth=syn, obj="kl", size="3M", subsample=subsample))
